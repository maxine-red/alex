...
Matrix::Matrix(unsigned int r, unsigned int c) : rows(r), columns(c) {
	...
	content.insert(content.begin(), 0.0, rows * columns);
	deltas.insert(deltas.begin(), 0.0, rows * columns);
}

void Matrix::randomize(double mu, double std) {
	std::default_random_engine generator;
	std::uniform_real_distribution<double> distribution(mu-std, mu+std);
	int r;
	for (int i = content.size(); i; --i) {
		do {
