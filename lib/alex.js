...
class Alex {
  ...
  constructor() {
    ...
    if (fs.existsSync(`${this.file}/model.json`) &&
      ...
      if (config.network === undefined) {
        config.network = {
          layers: [
            { units: 64, activation: 'tanh', inputShape: [1024]},
  ...
  learn() {
    ...
    if (config.network.train_parameters === undefined) {
      config.network.train_parameters = {
        epochs: 100,
        batchSize: 32,
        shuffle: true,
        verbose: 0
  ...
  load_model(model) {
    this.model = model;
    if (config.network.parameters !== undefined) {
