...
function on_connect(conn) {
  ...
  conn.on('data', function (data) {
    ...
    let message;
    switch(data.event) {
    case 'error': server.emit('error', { code: 'no data object', conn); break;
    case 'train': if (data.data === undefined) {
      server.emit('error', { code: 'no data object' }, conn);
    }
    else {
...
function on_error(err, conn) {
  if (conn === undefined) {
    ...
    message = { event: 'error', data: { message: 'malformed request' } };
    conn.write(JSON.stringify(message));
  }
}
